...
client.on('message', async message => {
    ...
    if(command == prefix + 'join-seller') {
        ...
        if(!subChannel) return message.channel.send(':x: | يجب ان يتوفر روم اسمه `قبول رفض`');
                    ...
                    message.channel.awaitMessages(filter, { max: 1, time: 30000, errors: ['time'] }).then(collected => {
                        ...
                        msgS.edit(':timer: | **من فضلك اكتب من اي بلد انت**').then(msgS => {
                            message.channel.awaitMessages(filter, { max: 1, time: 30000, errors: ['time'] }).then(collected => {
                                ...
                                collected.first().delete();
                                msgS.edit(':timer: | **اكتب وش تبيع من فضلك**').then(msgS => {
                                    message.channel.awaitMessages(filter, { max: 1, time: 30000, errors: ['time'] }).then(collected => {
